...
export async function createUser(req, res) {
  let { username, email, first_name, last_name, password, role, sendEmail } = req.body;
  ...
  const autogenerated = {};
  const plainPassword = password; // Store before hashing for email
  if (!username) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      try {
        ...
        // Send welcome email if requested and valid email provided
        if (sendEmail && normalizedEmail && !normalizedEmail.includes('@placeholder.local')) {
          try {
...
export async function getSubmissionDetail(req, res) {
  ...
  try {
    const subResult = await pool.query(
      `SELECT es.*, u.username, u.first_name, u.last_name, u.email, e.title AS exam_title
       FROM exam_submissions es
       JOIN users u ON u.id = es.user_id
       JOIN exams e ON e.id = es.exam_id
       WHERE es.id = $1`,
...
/**
 * Generate PDF for a submission (download)
 */
export async function generateSubmissionPDFDownload(req, res) {
  const { submissionId } = req.params;
  ...
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="exam-results-${submission.exam_title.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.pdf"`
    );
    res.send(pdfBuffer);
  } catch (err) {
    console.error("generateSubmissionPDFDownload error:", err);
    res.status(500).json({ error: "Failed to generate PDF" });
  }
}

/**
 * Send submission PDF via email
 */
export async function emailSubmissionPDF(req, res) {
  ...
  try {
    ...
    if (!submission.email || submission.email.includes('@placeholder.local')) {
      return res.status(400).json({ error: "No valid email address for this student" });
    }
    ...
    await sendSubmissionEmail(submission.email, {
    ...
    res.json({ success: true, message: `Email sent to ${submission.email}` });
  } catch (err) {
